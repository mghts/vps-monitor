...
1. 用 GitHub Actions 发布中心端和 Agent 的 Docker 镜像。
2. 在中心端 VPS 创建 `/docker/vps-monitor`。
3. 只放 `docker-compose.yml` 和 `.env` 两个文件。
4. 执行 `docker compose up -d`。
5. 配好 Nginx + HTTPS，然后在网页里首次注册管理员、添加节点、复制 Agent 安装命令。

生产部署不需要把 `server/`、`agent/`、`web/` 这些源码目录上传到 VPS。中心端镜像会内置后端和前端页面，Agent 镜像会内置 Agent 程序。
...
如果你只想本地看界面，用 `web` 目录的 Vite 开发服务即可；如果你要真实部署，按 [VPS 中心端部署教程](docs/vps-deploy.md) 走，不要把本地预览服务当成生产服务。
...
4. 推送 `v0.2.1` tag，GitHub Actions 会自动发布多架构 Docker 镜像和 Agent 二进制。
5. 在 VPS 的 `.env` 里填写：
...
管理后台会同时生成 Docker 版和 systemd/native 版 Agent 安装命令：

- Docker 版：目标 VPS 需要安装 Docker，只依赖 `AGENT_IMAGE` 指向的 Docker 镜像。推荐优先用这个方式，升级和卸载更简单。
- systemd/native 版：目标 VPS 不需要 Docker，但需要能从 GitHub Releases 下载 `vps-monitor-agent-linux-amd64`、`vps-monitor-agent-linux-arm64` 或 `vps-monitor-agent-linux-armv7`。如果没有发布这些 release assets，systemd 安装会失败。

两种方式都会使用每台服务器独立的 Agent token。token 只在创建或重置时显示一次，丢了就去管理后台“重置 token”。
...
- `--read-only`、`--cap-drop ALL`、`no-new-privileges`：限制容器写入和提权能力。
- 仅只读挂载 `/etc/hostname` 和 `/etc/os-release`；不会把宿主机整个 `/` 暴露给 Agent。
...
export SETUP_TOKEN=dev-setup-token-change-me-32-chars
...
- 认证入口带失败限流；`SETUP_TOKEN` 至少 32 个字符，建议用 `openssl rand -hex 32` 生成。
- 建议中心端只通过 HTTPS 反向代理对外服务，保持 `SERVER_PORT=127.0.0.1:8080`，不要向公网开放 8080。
- Docker Agent 采用隔离网络和只读文件系统，只保留 `NET_RAW`，仅只读挂载主机名、系统版本与 `/proc/net/dev`，不挂载宿主机整个根目录；systemd Agent 下载二进制时会验证 SHA-256。
